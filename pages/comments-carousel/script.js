...
class Carousel {
   constructor() {
      ...
      //this.listItems = document.querySelectorAll('.comments-carousel__item');
      this.listItems = document.querySelectorAll('.comments__card');
      this.prev = document.querySelector('.comments-prev');
      this.next = document.querySelector('.comments-next');
   ...
   getConfig() {
      ...
      let widthItemWithMargin = this.listItems[0].getBoundingClientRect().width;//ширина слайда + марджин
      let count = Math.round(widthGallery / widthItemWithMargin);//количество видимых слайдов 
      ...
      let caruselll = document.querySelector('.comments-carousel__items');
      let gap = +window.getComputedStyle(caruselll).gap.slice(0,-2);
      return {count: count, widthItemWithMargin: widthItemWithMargin+gap, widthItem: widthItem};
   ...
   colorArrow() {
      ...
      }else if(this.itemIndex === this.listItems.length - config.count*3) {
         this.next.classList.add('inactive');
      }else if (this.itemIndex !== 0 && this.itemIndex !== this.listItems.length - config.count) {
   ...
   smartIncrement() {
      let config = this.getConfig();
      if(this.itemIndex !== this.listItems.length - config.count) {
   ...
   carouselInit() {
      ...
      let that = this;   
      window.addEventListener('resize', (e)=>{that.show()});
...
let commentsCarousel = new Carousel();
commentsCarousel.carouselInit();
